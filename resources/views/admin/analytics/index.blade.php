...
@section('title', __('messages.intelligence'))
...
<script>
    ...
    // Export function supporting CSV, PDF, and DOCX generation
    function downloadFormat(format) {
        if (format === 'csv') {
            const link = document.createElement('a');
            link.href = "{{ route('admin.analytics.export') }}";
            link.download = 'veltrix_intelligence_report.csv';
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
            showSuccessMessage("Workspace intelligence report exported as CSV successfully!");
        } else if (format === 'docx') {
            // Generate a beautifully styled DOCX report (using formatted HTML structure)
            const htmlContent = `
                <html xmlns:o='urn:schemas-microsoft-com:office:office' xmlns:w='urn:schemas-microsoft-com:office:word' xmlns='http://www.w3.org/TR/REC-html40'>
                <head>
                    <title>Veltrix CRM Intelligence Report</title>
                    <style>
                ...
                <body>
                    <h1>VELTRIX CRM REPORT</h1>
                    <p><strong>Generated At:</strong> ${new Date().toLocaleString()}</p>
                    <p><strong>Author:</strong> ${document.querySelector('#topbar .font-bold')?.textContent || 'Administrator'}</p>
                    
                    <h3>1. Customer Entity Distribution</h3>
                    <table>
                        <thead>
                            <tr>
                    ...
                    <h3>3. Recent Operational Activity Logs</h3>
                    <table>
                        <thead>
                            <tr>
                                <th>User</th>
            ...
            const link = document.createElement('a');
            link.href = url;
            link.download = 'veltrix_intelligence_report_' + new Date().toISOString().slice(0,10) + '.docx';
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
            ...
            element.innerHTML = `
                <div style="border-bottom: 2px solid #2D3A2D; padding-bottom: 16px; margin-bottom: 24px;">
                    ...
                    <p style="color: #706F6C; margin: 4px 0 0 0; font-size: 13px;">Business Intelligence & Performance Report</p>
                </div>
                ...
                <table style="width: 100%; margin-bottom: 20px; font-size: 12px; color: #555;">
                    <tr>
                        ...
                        <td style="text-align: right;"><strong>Author:</strong> ${document.querySelector('#topbar .font-bold')?.textContent || 'Administrator'}</td>
                    </tr>
                </table>

                <div style="margin-top: 20px; margin-bottom: 24px;">
                    <h3 style="color: #2D3A2D; border-bottom: 1px solid #E5E2DC; padding-bottom: 6px; font-size: 16px;">1. Customer Entity Distribution</h3>
                ...
                <div style="margin-top: 20px; margin-bottom: 24px;">
                    <h3 style="color: #2D3A2D; border-bottom: 1px solid #E5E2DC; padding-bottom: 6px; font-size: 16px;">2. Operational Velocity</h3>
                    <table style="width: 100%; border-collapse: collapse; margin-top: 10px; font-size: 13px;">
                        <thead>
                            <tr style="border-bottom: 1.5px solid #2D3A2D; text-align: left; font-weight: bold; color: #2D3A2D;">
                ...
                <div style="margin-top: 20px; margin-bottom: 10px; page-break-inside: avoid;">
                    <h3 style="color: #2D3A2D; border-bottom: 1px solid #E5E2DC; padding-bottom: 6px; font-size: 16px;">3. Recent Operational Activity Logs</h3>
            ...
            const opt = {
                margin: 0.5,
                filename: 'veltrix_intelligence_report_' + new Date().toISOString().slice(0,10) + '.pdf',
        ...
        customerChart = new Chart(ctxC, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    backgroundColor: ['#d97706', '#065f46', '#71717a'],
                    borderWidth: 0,
                    hoverOffset: 15
                }]
            },
        ...
        taskChart = new Chart(ctxT, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    backgroundColor: ['#10b981', '#059669', '#064e3b'],
                    borderRadius: 12,
                    maxBarThickness: 45
                }]
            },
    ...
    async function clearLogs() {
        ...
        try {
            const response = await fetch("{{ route('admin.analytics.logs.clear') }}", {
                method: 'DELETE',
                headers: {
