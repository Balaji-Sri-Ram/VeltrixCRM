...
<div class="mb-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
    <div>
        <h1 class="heading-editorial text-4xl mb-1">{{ __('messages.my_customers') }}</h1>
        <p class="text-muted-veltrix text-sm">{{ __('messages.staff_overview') }}</p>
    </div>
    <div class="flex flex-wrap items-center gap-4 w-full md:w-auto">
        <form action="{{ route('staff.customers') }}" method="GET" class="flex flex-1 md:flex-none items-center gap-3">
            <div class="relative flex-1 md:w-72">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ __('messages.search_assigned_records') }}" class="w-full bg-white border border-[var(--color-border-soft)] rounded-2xl px-5 py-3 text-xs font-bold uppercase tracking-widest focus:border-[var(--color-primary)] outline-none shadow-sm transition-all">
            ...
            @if(request('search'))
                <a href="{{ route('staff.customers') }}" class="text-muted-veltrix hover:text-[var(--color-accent)] p-2 transition-colors" title="Clear Filters">
        ...
        <button id="open-modal-btn" class="btn-primary-veltrix space-x-2">
            ...
            <span class="text-xs uppercase tracking-widest font-bold">{{ __('messages.add_customer') }}</span>
        </button>
    </div>
</div>
            ...
            <tbody id="customer-table-body">
                ...
                <tr>
                    <td colspan="5" class="px-8 py-20 text-center">
                        ...
                        <p class="text-muted-veltrix text-sm font-bold uppercase tracking-widest opacity-40">No assigned records found</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
...
<div id="add-modal" class="fixed inset-0 z-[100] hidden flex items-center justify-center bg-[var(--color-primary)]/10 backdrop-blur-md">
    <div class="bg-white w-full max-w-lg rounded-[32px] shadow-2xl overflow-hidden transform scale-95 opacity-0 transition-all duration-300 border border-[var(--color-border-soft)]" id="modal-content">
        <div class="px-10 py-8 border-b border-[var(--color-border-soft)] flex justify-between items-center bg-[var(--color-bg-card)]">
            <div>
                <h3 class="heading-editorial text-2xl">New Entity</h3>
                <p class="text-[10px] font-bold text-muted-veltrix uppercase tracking-widest mt-1">Direct Assignment Onboarding</p>
            </div>
        ...
        <div class="p-10">
            <form id="add-customer-form" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" required class="w-full bg-[var(--color-bg-card)] border border-[var(--color-border-soft)] rounded-2xl px-5 py-3.5 outline-none focus:border-[var(--color-primary)] transition-all text-sm font-bold" placeholder="Full name">
                </div>
                <div>
                    <label class="block text-[10px] font-bold text-muted-veltrix uppercase tracking-widest mb-3">{{ __('messages.email_address') }}</label>
                    <input type="email" name="email" class="w-full bg-[var(--color-bg-card)] border border-[var(--color-border-soft)] rounded-2xl px-5 py-3.5 outline-none focus:border-[var(--color-primary)] transition-all text-sm font-bold" placeholder="email@example.com">
                </div>
                <div>
                    <label class="block text-[10px] font-bold text-muted-veltrix uppercase tracking-widest mb-3">{{ __('messages.phone_number') }}</label>
                    <input type="text" name="phone" class="w-full bg-[var(--color-bg-card)] border border-[var(--color-border-soft)] rounded-2xl px-5 py-3.5 outline-none focus:border-[var(--color-primary)] transition-all text-sm font-bold" placeholder="+1 234 567 8900">
                </div>
                <div>
                    <label class="block text-[10px] font-bold text-muted-veltrix uppercase tracking-widest mb-3">{{ __('messages.status') }}</label>
                    <select name="status" required class="w-full bg-[var(--color-bg-card)] border border-[var(--color-border-soft)] rounded-2xl px-5 py-3.5 outline-none focus:border-[var(--color-primary)] transition-all text-xs font-bold uppercase tracking-widest appearance-none cursor-pointer">
...
<script>
    ...
    addForm?.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (data.success) {
                closeModal();
                showSuccessMessage('Entity registered successfully');
