...
<body class="bg-[var(--color-bg-base)] text-[var(--color-charcoal)]">
    ...
    <!-- Top Navbar -->
    <nav class="w-full py-6 sticky top-0 z-50 bg-[var(--color-bg-base)]/80 backdrop-blur-md border-b border-[var(--color-border-soft)]">
        <div class="container-boxed flex items-center justify-between">
            ...
            <div class="flex items-center gap-4">
                <div class="hidden sm:flex items-center gap-6">
                    ...
                    @else
                        <a href="{{ route('login') }}" class="text-xs font-bold uppercase tracking-widest text-[var(--color-graphite)] hover:text-[var(--color-charcoal)] transition-colors">{{ __('messages.login') }}</a>
                        <a href="{{ route('register') }}" class="btn-primary-veltrix !py-2.5 !px-6 text-xs font-bold uppercase tracking-widest">
        ...
        <!-- Mobile Menu Overlay -->
        <div id="mobile-menu" class="fixed inset-0 bg-white z-[60] flex flex-col items-center justify-center gap-8 translate-x-full transition-transform duration-500 md:hidden">
            ...
            <div class="pt-8 flex flex-col gap-4 w-full px-12">
                @auth
                    <a href="{{ auth()->user()->role === 'admin' ? route('admin.dashboard') : route('staff.dashboard') }}" class="btn-primary-veltrix w-full text-center !py-5">{{ __('messages.dashboard') }}</a>
                @else
                    <a href="{{ route('login') }}" class="btn-secondary-veltrix w-full text-center !py-5">{{ __('messages.login') }}</a>
                    <a href="{{ route('register') }}" class="btn-primary-veltrix w-full text-center !py-5">{{ __('messages.get_started') }}</a>
                @endauth
            </div>
        </div>
    </nav>
    ...
    <main>
        <section class="pt-2 pb-24 lg:pt-4 lg:pb-32 overflow-hidden">
            <div class="container-boxed grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
                ...
                <div class="space-y-8 animate-hero-text">
                    <div class="inline-flex items-center gap-3 px-4 py-1.5 rounded-full bg-white border border-[var(--color-border-soft)] text-[var(--color-primary)] font-bold text-[10px] tracking-[0.2em] uppercase shadow-sm">
                    ...
                    <h1 class="heading-editorial text-5xl lg:text-7xl leading-[1.1]">
                        {{ __('messages.hero_heading_1') }} <span class="text-[var(--color-primary)]">{{ __('messages.hero_heading_2') }}</span>
                    </h1>
                    ...
                    <div class="flex flex-wrap gap-5 pt-4">
                        @auth
                            <a href="{{ auth()->user()->role === 'admin' ? route('admin.dashboard') : route('staff.dashboard') }}" class="btn-primary-veltrix text-xs uppercase tracking-widest px-10 !py-4">
                                {{ __('messages.get_started') }}
                            </a>
                            <a href="{{ auth()->user()->role === 'admin' ? route('admin.dashboard') : route('staff.dashboard') }}" class="btn-secondary-veltrix text-xs uppercase tracking-widest px-10 !py-4 bg-white border border-[var(--color-border-soft)]">
                                {{ __('messages.dashboard') }}
                            </a>
                        @else
                            <a href="{{ route('register') }}" class="btn-primary-veltrix text-xs uppercase tracking-widest px-10 !py-4">
                                {{ __('messages.get_started') }}
                            </a>
                            <a href="{{ route('login') }}" class="btn-secondary-veltrix text-xs uppercase tracking-widest px-10 !py-4 bg-white border border-[var(--color-border-soft)]">
                                {{ __('messages.login') }}
                            </a>
                        @endauth
                    </div>
            ...
            <div class="container-boxed">
                ...
                <div class="relative w-full mt-16 min-h-[350px]">
                    ...
                    <div class="features-row-1 grid md:grid-cols-3 gap-6 w-full relative z-10 opacity-0">
                        ...
                        <div class="card-veltrix hover:-translate-y-2 transition-all duration-500 group border-[var(--color-border-soft)]">
                            <div class="w-14 h-14 bg-[var(--color-bg-base)] rounded-2xl border border-[var(--color-border-soft)] flex items-center justify-center text-[var(--color-primary)] mb-6 group-hover:bg-[var(--color-primary)] group-hover:text-white transition-colors duration-500">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="{{ $f['icon'] }}"></path></svg>
                            </div>
                    ...
                    <div class="features-row-2 grid md:grid-cols-3 gap-6 w-full absolute inset-x-0 top-0 opacity-0 pointer-events-none z-20">
        ...
        <section id="contact" class="scroll-section py-32 bg-[var(--color-bg-card)] border-y border-[var(--color-border-soft)]">
            <div class="container-boxed">
                <div class="grid lg:grid-cols-2 gap-20 items-center">
                    <div>
                        <h2 class="heading-editorial text-4xl lg:text-5xl mb-6">{{ __('messages.contact_heading') }}</h2>
                        <p class="text-muted-veltrix text-lg mb-10 max-w-lg">{{ __('messages.contact_desc') }}</p>
                        
                        <div class="space-y-8">
                            <div class="flex items-start gap-5">
                                <div class="w-12 h-12 rounded-xl bg-white border border-[var(--color-border-soft)] flex items-center justify-center text-[var(--color-primary)] shadow-sm">
                            ...
                            <div class="flex items-start gap-5">
                                ...
                                <div>
                                    <h4 class="text-xs font-bold uppercase tracking-widest text-[var(--color-charcoal)] mb-1">{{ __('messages.direct_line') }}</h4>
                    ...
                    <div class="card-veltrix bg-white !p-10 shadow-xl border-[var(--color-border-soft)] relative overflow-hidden">
                        <!-- Success Message (hidden by default) -->
                        <div id="contact-success-msg" class="hidden">
                            <div class="text-center py-8">
                                <div class="w-20 h-20 bg-emerald-50 border-2 border-emerald-200 rounded-full flex items-center justify-center mx-auto mb-6 animate-bounce-in">
                                ...
                                <h3 class="heading-editorial text-2xl mb-3 text-[var(--color-charcoal)]">Query Transmitted Successfully</h3>
                                <p class="text-muted-veltrix text-sm max-w-sm mx-auto mb-8">Thank you for reaching out. Our team will review your inquiry and respond within 24 hours.</p>
                                <button onclick="resetContactForm()" class="btn-secondary-veltrix text-xs uppercase tracking-widest font-bold">
                                    Send Another Query
                                </button>
                            </div>
                        </div>
                        ...
                        <div id="contact-error-msg" class="hidden mb-6 p-4 rounded-2xl bg-red-50 border border-red-200 text-red-800 text-sm font-semibold flex items-center gap-3">
                        ...
                        <form id="contact-form" action="{{ route('contact.submit') }}" method="POST" class="space-y-6">
                            ...
                            <div class="grid md:grid-cols-2 gap-6">
                                <div>
                                    <label class="block text-[10px] font-bold text-muted-veltrix uppercase tracking-widest mb-3">{{ __('messages.your_name') }}</label>
                                    <input type="text" name="name" required class="w-full bg-[var(--color-bg-base)] border border-[var(--color-border-soft)] rounded-2xl px-5 py-4 outline-none focus:border-[var(--color-primary)] transition-all text-sm font-bold" placeholder="E.g. Ramu Parasa">
                                </div>
                                <div>
                                    <label class="block text-[10px] font-bold text-muted-veltrix uppercase tracking-widest mb-3">{{ __('messages.email_address') }}</label>
                                    <input type="email" name="email" required class="w-full bg-[var(--color-bg-base)] border border-[var(--color-border-soft)] rounded-2xl px-5 py-4 outline-none focus:border-[var(--color-primary)] transition-all text-sm font-bold" placeholder="email@enterprise.com">
                                </div>
                            </div>
                            <div>
                                <label class="block text-[10px] font-bold text-muted-veltrix uppercase tracking-widest mb-3">{{ __('messages.message') }}</label>
                                <textarea rows="5" name="message" required class="w-full bg-[var(--color-bg-base)] border border-[var(--color-border-soft)] rounded-2xl px-5 py-4 outline-none focus:border-[var(--color-primary)] transition-all text-sm font-medium leading-relaxed" placeholder="{{ __('messages.message_placeholder') }}"></textarea>
                            </div>
                            <button type="submit" id="contact-submit-btn" class="w-full btn-primary-veltrix !py-5">
                                <span id="contact-btn-text" class="text-xs uppercase tracking-widest font-bold">{{ __('messages.submit_query') }}</span>
                                <svg id="contact-btn-spinner" class="hidden w-5 h-5 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                            </button>
                        </form>
                    </div>
                </div>
        ...
        <section class="scroll-section py-32 bg-[var(--color-primary)] text-white overflow-hidden relative">
            ...
            <div class="container-boxed text-center relative z-10">
                ...
                <p class="text-emerald-100 text-xl max-w-2xl mx-auto mb-12 font-medium opacity-80">{{ __('messages.cta_desc') }}</p>
                <div class="flex flex-wrap justify-center gap-6">
                    <a href="{{ auth()->check() ? route('pricing') : route('register') }}" class="inline-flex items-center justify-center bg-white text-[var(--color-primary)] hover:bg-[var(--color-bg-base)] px-12 py-5 rounded-2xl text-xs font-bold uppercase tracking-widest shadow-xl transition-all hover:-translate-y-1">
                        {{ __('messages.cta_button') }}
                    </a>
                </div>
                <p class="mt-10 text-[10px] font-bold text-emerald-200 uppercase tracking-widest opacity-60">{!! __('messages.cta_subtext') !!}</p>
            </div>
        </section>
    </main>
            ...
            <div class="grid md:grid-cols-4 gap-16 mb-20">
                <div class="col-span-2">
                    <div class="flex items-center gap-2 mb-8">
                        <div class="w-9 h-9 bg-[var(--color-primary)] rounded-lg flex items-center justify-center text-white font-bold text-lg">V</div>
                ...
                <div>
                    <h5 class="text-[10px] font-bold uppercase tracking-widest text-[var(--color-charcoal)] mb-8">{{ __('messages.footer_col_1') }}</h5>
                    <ul class="space-y-4 text-xs font-bold text-muted-veltrix uppercase tracking-[0.15em]">
                        <li><a href="#features" class="hover:text-[var(--color-primary)] transition-colors">{{ __('messages.features') }}</a></li>
                        <li><a href="{{ route('pricing') }}" class="hover:text-[var(--color-primary)] transition-colors">{{ __('messages.pricing') }}</a></li>
                        <li><a href="#" class="hover:text-[var(--color-primary)] transition-colors">{{ __('messages.enterprise') }}</a></li>
                    </ul>
                </div>
                <div>
